...
- **디렉터리 구조**: `src` (소스 파일), `include` (헤더 파일), `tests` (테스트 파일) 등으로 분리.

#### 2. 코드 레이아웃
- **들여쓰기**: 4칸 공백 사용.
- **행 길이**: 최대 80자.
- **중괄호 스타일**: 새로운 줄에 중괄호를 염.
  ```c
  if (condition)
  {
      // code
...
#### 3. 변수 및 함수 명명 규칙
- **변수 이름**: 소문자와 밑줄(`_`) 사용. 예: `int counter;`.
- **전역 변수**: 접두사 `g_` 사용. 예: `int g_total;`.
- **함수 이름**: 소문자와 밑줄(`_`) 사용. 예: `void calculate_sum();`.
- **매크로 상수**: 대문자와 밑줄(`_`) 사용. 예: `#define MAX_BUFFER_SIZE 1024`.

#### 4. 주석
- **단일 행 주석**: `//` 사용.
- **다중 행 주석**: `/* ... */` 사용.
- **함수 주석**: 함수 설명, 인자, 반환 값 기재.
  ```c
  /**
   * 함수 설명: 두 정수를 더함
   * @param a 첫 번째 정수
...
#### 5. 코드 조직화
- **헤더 파일**: 함수 프로토타입과 전역 변수 선언 포함.
- **파일 포함 순서**: 표준 라이브러리 헤더 -> 써드파티 라이브러리 헤더 -> 프로젝트 헤더 파일.
  ```c
  #include <stdio.h>
  #include "utils.h"
  ```
...
#### 8. 안전한 프로그래밍
- **변수 초기화**: 모든 변수를 초기화.
- **경계 검사**: 배열 및 포인터 사용 시 경계 검사 철저.
- **NULL 검사**: 포인터 사용 전 NULL 검사.

#### 9. 버전 관리
- **버전 주석**: 파일 상단에 버전 정보와 작성자 정보 포함.
  ```c
  /**
   * 파일: 파일명.c
   * 작성자: 이름
...
#### 10. 테스트
- **단위 테스트**: 각 함수에 대해 단위 테스트 작성 및 주기적 실행.
...
1. **일관성 유지**: 프로젝트 전반에 걸쳐 일관된 주석 스타일 유지.
2. **명확하고 간결하게 작성**: 주석을 명확하고 간결하게 작성.
3. **필요한 부분에만 주석 작성**: 이해하기 어려운 부분이나 중요한 로직에만 주석 작성.
4. **코드 수정 시 주석 업데이트**: 코드 수정 시 주석도 함께 업데이트.
